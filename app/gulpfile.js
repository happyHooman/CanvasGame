...
function slackNotifyDeploy(){
  ...
  var media_only = "";
  if(argv['media']){
    media_only = ' (IMAGES AND FONTS ONLY) ';
  }
  var msg = user_details + " deployed `" + projectName() + media_only + "` to " + deploy_env + " with SDK version " + getPublishVersion();
...
gulp.task('list-deps', ['list-all-dependencies'],  function(cb){
  ...
  if(deploy_env && deploy_env.match(/(prod|stag)/i)){
    // empty bower_list.js in javascripts folder
    fs.writeFile('./src/javascripts/bower_list.js', '');
  }

  if(dependencies_saved || (deploy_env && deploy_env.match(/(prod|stag)/i))){
    cb();
    return;
  }
  ...
  if(!bug_png){
    gulp.src('../project_template/src/images/bug.png')
...
gulp.task('templates', ['config-set'], function() {
  return gulp.src('./src/templates/*.html')
    .pipe(templateCache('x_templates.js', {module: 'TemplateOverrides', standalone: true}))
    .pipe(flatten())
    //.pipe(template(require(config)))
    .pipe(template(config))
    .pipe(gulp.dest('tmpjs'));
